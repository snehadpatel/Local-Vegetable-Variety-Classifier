...
from models import CustomMultiScaleCNN
...
EPOCHS = 10
LEARNING_RATE = 1e-4
if 'CustomMultiScaleCNN' == 'CustomDeepCNN':
    EPOCHS = 8
...
def main():
    ...
    # 4. Instantiate Model
    model = CustomMultiScaleCNN(num_classes=len(classes)).to(device)
    ...
    early_stopping = EarlyStopping(patience=3, verbose=True, path="multiscale_model.pth")
    ...
    print("\n" + "="*50)
    print("           TRAINING CUSTOMMULTISCALECNN (MODEL 4)")
    print("="*50)
    ...
    plt.title("Loss Curves (CustomMultiScaleCNN)")
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.legend()
    plt.grid(True)
    ...
    plt.title("Accuracy Curves (CustomMultiScaleCNN)")
    plt.xlabel("Epoch")
    plt.ylabel("Accuracy")
    plt.legend()
    plt.grid(True)
    ...
    plt.tight_layout()
    plt.savefig("multiscale_curves.png")
    print("Saved learning curves as 'multiscale_curves.png'.")
    ...
    model.load_state_dict(torch.load("multiscale_model.pth"))
    model.eval()
