...
from models import CustomDeepCNN
...
EPOCHS = 10
LEARNING_RATE = 1e-4
if 'CustomDeepCNN' == 'CustomDeepCNN':
    EPOCHS = 8
...
def main():
    ...
    # 4. Instantiate Model
    model = CustomDeepCNN(num_classes=len(classes)).to(device)
    ...
    early_stopping = EarlyStopping(patience=3, verbose=True, path="deep_model.pth")
    ...
    print("\n" + "="*50)
    print("           TRAINING CUSTOMDEEPCNN (MODEL 2)")
    print("="*50)
    ...
    plt.title("Loss Curves (CustomDeepCNN)")
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.legend()
    plt.grid(True)
    ...
    plt.title("Accuracy Curves (CustomDeepCNN)")
    plt.xlabel("Epoch")
    plt.ylabel("Accuracy")
    plt.legend()
    plt.grid(True)
    ...
    plt.tight_layout()
    plt.savefig("deep_curves.png")
    print("Saved learning curves as 'deep_curves.png'.")
