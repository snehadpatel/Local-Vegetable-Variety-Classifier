...
from models import CustomLightCNN
...
EPOCHS = 10
LEARNING_RATE = 1e-4
if 'CustomLightCNN' == 'CustomDeepCNN':
    EPOCHS = 8
...
def main():
    ...
    # 4. Instantiate Model
    model = CustomLightCNN(num_classes=len(classes)).to(device)
    ...
    early_stopping = EarlyStopping(patience=3, verbose=True, path="light_model.pth")
    ...
    print("\n" + "="*50)
    print("           TRAINING CUSTOMLIGHTCNN (MODEL 1)")
    print("="*50)
    ...
    plt.title("Loss Curves (CustomLightCNN)")
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.legend()
    plt.grid(True)
    ...
    plt.title("Accuracy Curves (CustomLightCNN)")
    plt.xlabel("Epoch")
    plt.ylabel("Accuracy")
    plt.legend()
    plt.grid(True)
    ...
    print("Saved learning curves as 'light_curves.png'.")
    ...
    model.load_state_dict(torch.load("light_model.pth"))
    model.eval()
