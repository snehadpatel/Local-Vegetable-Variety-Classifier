...
from models import CustomDepthwiseSepCNN
...
EPOCHS = 10
LEARNING_RATE = 1e-4
if 'CustomDepthwiseSepCNN' == 'CustomDeepCNN':
    EPOCHS = 8
...
def main():
    ...
    # 4. Instantiate Model
    model = CustomDepthwiseSepCNN(num_classes=len(classes)).to(device)
    ...
    early_stopping = EarlyStopping(patience=3, verbose=True, path="depthwise_model.pth")
    ...
    print("\n" + "="*50)
    print("           TRAINING CUSTOMDEPTHWISESEPCNN (MODEL 6)")
    print("="*50)
    ...
    plt.title("Loss Curves (CustomDepthwiseSepCNN)")
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.legend()
    plt.grid(True)
    ...
    plt.title("Accuracy Curves (CustomDepthwiseSepCNN)")
    plt.xlabel("Epoch")
    plt.ylabel("Accuracy")
    plt.legend()
    plt.grid(True)
    ...
    plt.tight_layout()
    plt.savefig("depthwise_curves.png")
    print("Saved learning curves as 'depthwise_curves.png'.")
    ...
    model.load_state_dict(torch.load("depthwise_model.pth"))
    model.eval()
