...
from models import CustomGAPCNN
...
def main():
    ...
    # 4. Instantiate Model
    model = CustomGAPCNN(num_classes=len(classes)).to(device)
    ...
    early_stopping = EarlyStopping(patience=3, verbose=True, path="gap_model.pth")
    ...
    print("\n" + "="*50)
    print("           TRAINING CUSTOMGAPCNN (MODEL 3)")
    print("="*50)
    ...
    plt.title("Loss Curves (CustomGAPCNN)")
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.legend()
    plt.grid(True)
    ...
    plt.title("Accuracy Curves (CustomGAPCNN)")
    plt.xlabel("Epoch")
    plt.ylabel("Accuracy")
    plt.legend()
    plt.grid(True)
    ...
    plt.tight_layout()
    plt.savefig("gap_curves.png")
    print("Saved learning curves as 'gap_curves.png'.")
    ...
    model.load_state_dict(torch.load("gap_model.pth"))
    model.eval()
